...
import at.ac.tuwien.dbai.hgtools.sql2hg.sqlshare.AddAliasTransformer;
import at.ac.tuwien.dbai.hgtools.sql2hg.sqlshare.PredicateBuilder;
import at.ac.tuwien.dbai.hgtools.sql2hg.sqlshare.QuoteWrapper;
import at.ac.tuwien.dbai.hgtools.sql2hg.sqlshare.SchemaMaker;
...
public class MainMakeQuery {
	...
	public static void main(String[] args) throws JSQLParserException, IOException {
		...
		Util.loadListMap(tab2users, tab2userName, true, false);
		Util.loadListMap(user2tabs, user2tabName, false, true);
		Util.loadSimpleMap(view2file, view2fileName, true, false);
		Util.loadSimpleMap(file2view, file2viewName, false, true);
		...
		for (File file : files) {
			if (file.isDirectory()) {
				...
				if (stmts.getStatements().size() != 1) {
					throw new RuntimeException("Multiple statements");
				}
				Statement stmt = stmts.getStatements().get(0);
				Select sel = (stmt instanceof Select) ? (Select) stmt : getSelect(stmt);
				if (sel == null) {
					continue;
				}

				// find external views and add them to sel
				...
				LinkedList<Select> selects = new LinkedList<>();
				selects.add(sel);
				while (!selects.isEmpty()) {
					Select s = selects.removeFirst();
					HashSet<String> tables = tabFinder.findTables(s);

					if (s.getWithItemsList() != null) {
						items.addAll(s.getWithItemsList());
						s.getWithItemsList().forEach(new Consumer<WithItem>() {
							public void accept(WithItem v) {
								String vName = v.getName();
								if (itemNames.contains(vName)) {
									throw new RuntimeException(vName + " already exists.");
								}
								itemNames.add(vName);
							}
						});
					}
					// assuming no WithItem in CreateView
					for (String tab : tables) {
						String t = Util.addQuote(tab);
						if (view2file.containsKey(t)) {
							if (!(itemNames.contains(tab) || itemNames.contains(t))) {
								String viewFile = view2file.get(t);
								CreateView cv = getView(viewFile);
								WithItem v = makeWithItem(cv);
								items.add(v);
								itemNames.add(v.getName());
								selects.addLast(cv.getSelect());
							}
						}
					}
				}
				sel.setWithItemsList(items);

				// add aliases to anonymous subqueries
				AddAliasTransformer at = new AddAliasTransformer();
				at.run(sel);
				// add quotes everywhere
				QuoteWrapper qe = new QuoteWrapper();
				qe.run(sel);
				// make name lower case
				ToLowerCaseTransformer lc = new ToLowerCaseTransformer();
				lc.run(sel);

				// build predicate stubs for the complete query
				PredicateBuilder pb = new PredicateBuilder("", null, itemNames);
				pb.buildPredicates(sel);

				// resolve ambiguities
				SchemaMaker ar = new SchemaMaker(schema, tab2users, user2tabs, view2file, file2view);
				Schema outSchema = ar.getSchema(pb);
				...
				String qOut = outDir + File.separator + "complete_" + query;
				System.out.println(qOut);
				Util.writeToFile(qOut, sel);

				String schOut = outDir + File.separator + "schema_" + query;
	...
	private static Select getSelect(Statement stmt) {
		if (stmt instanceof CreateView) {
			return ((CreateView) stmt).getSelect();
		} else if (stmt instanceof Update) {
			return ((Update) stmt).getSelect();
		} else if (stmt instanceof CreateTable) {
			return ((CreateTable) stmt).getSelect();
		} else {
			throw new UnsupportedOperationException(stmt.getClass() + " not supported.");
		}
	}

	/**
	 * HashSet<String> visited = new HashSet<>(); recs.add(new Record(sel)); for
	 * (int i = 0; i < recs.size(); i++) { Record r = recs.get(i); HashSet<String>
	 * tables = tabFinder.findTables(sel); processRecord(r, tables); r.preds =
	 * pb.buildPredicates(r.stmt); r.subPreds = pb.getSubPredicates();
	 * r.unknownColumns = pb.getUnknowncolumns(); r.mySignature =
	 * pb.getMySignature();
	 * 
	 * for (String view : r.viewTables) { if (!visited.contains(view)) { CreateView
	 * vStmt = getView(view); recs.add(new Record(vStmt)); } }
	 * visited.addAll(r.viewTables); }
	 * 
	 * // TODO elabora i subPreds a un certo punto
	 * 
	 * expandPartialPredicates(schema); resolveAmbiguities(schema); clean();
	 * 
	 * HashSet<PredicateDefinition> allPreds = new HashSet<PredicateDefinition>();
	 * for (Record r : recs) { allPreds.addAll(r.preds);
	 * allPreds.addAll(r.undefinedPreds); } Schema outSchema = new Schema(); for
	 * (PredicateDefinition p : allPreds) { if (p.arity() == 0) { String name =
	 * p.getName(); LinkedList<String> attrs = new LinkedList<>();
	 * attrs.add("\"\""); p = new PredicateDefinition(name, attrs); }
	...
	private static CreateView getView(String vFile) throws JSQLParserException {
